...
export async function createTalkingVideo(photoUrl: string, text: string): Promise<string | null> {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn('[D-ID] Create failed:', res.status, await res.text().catch(() => ''));
      return null;
    }
