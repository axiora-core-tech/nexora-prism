...
import { Activity, TrendingUp, AlertTriangle, BrainCircuit, DollarSign, Target, Users, Zap, ArrowLeft, Brain, Lightbulb, Flame, Eye, Sparkles, ChevronRight } from 'lucide-react';
...
export function Analytics() {
  ...
  const insights = useMemo(() => {
    ...
    const totalValue = employees.reduce((s,e) => s+e.revenueContribution, 0);
    const totalCost = employees.reduce((s,e) => s+e.costInvestment, 0);
    ...
    if (burnoutRisk.length > 0) items.push({
      type:'critical', color:'#f43f5e', icon: Flame,
      title: `${burnoutRisk.length} employee${burnoutRisk.length>1?'s':''} in burnout corridor`,
      detail: `High output (>85pt) with low wellbeing (<70pt). This gap is unsustainable.`,
      action: `Schedule wellbeing check-ins with ${burnoutRisk.map(e=>e.name.split(' ')[0]).join(', ')} this week.`,
      empId: burnoutRisk[0].id,
    });
    if (highRisk.length > 0) items.push({
      type:'critical', color:'#fb923c', icon: AlertTriangle,
      title: `${highRisk.length} high-value flight risk${highRisk.length>1?'s':''}`,
      detail: `Combined ROI of ${highRisk.reduce((s,e)=>s+e.roi,0)}%. Replacement cost ≈$${Math.round(highRisk.reduce((s,e)=>s+e.costInvestment*1.5,0)/1000)}K.`,
      action: `Trigger retention protocol. ${highRisk[0].name.split(' ')[0]} should be highest priority.`,
      empId: highRisk[0].id,
    });
    if (avgROI > 200) items.push({
      type:'opportunity', color:'#10b981', icon: Sparkles,
      title: `Org ROI at ${avgROI}% — expansion territory`,
      detail: `Every $1 invested returns $${(avgROI/100).toFixed(1)}. The team is generating ${Math.round(totalValue/totalCost*100)}% return.`,
      action: `Consider increasing investment in top performers. ${topROI.name.split(' ')[0]} at ${topROI.roi}% ROI is your highest leverage.`,
    });
    if (lowWell.length > 0) items.push({
      type:'watch', color:'#f59e0b', icon: Eye,
      title: `${lowWell.length} employee${lowWell.length>1?'s':''} below wellbeing threshold`,
      detail: `Wellbeing below 65pt. This precedes motivation drops by 2-3 months.`,
      action: `Review workload distribution and consider forced PTO for ${lowWell.map(e=>e.name.split(' ')[0]).join(', ')}.`,
    });
    if (avgPerf > 85) items.push({
      type:'opportunity', color:'#38bdf8', icon: TrendingUp,
      title: `Team output at ${avgPerf}pt — above target`,
      detail: `${employees.filter(e=>e.performanceScore>=85).length} of ${employees.length} employees exceeding 85pt threshold.`,
      action: `Lock in gains: document processes, cross-train, and prepare stretch assignments.`,
    });
      ...
      {insights.length > 0 && (
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <p className="text-xs font-mono uppercase tracking-[0.2em] p-text-ghost">Insights & recommended actions</p>
          </div>
            ...
            {insights.slice(0, 4).map((ins, i) => {
              ...
              return (
                <motion.div key={i}
                  ...
                  className="rounded-xl p-4 group transition-all duration-300 hover:scale-[1.005]"
                  style={{ background: `${ins.color}06`, border: `1px solid ${ins.color}12`, cursor: ins.empId ? 'pointer' : 'default' }}
                  onClick={() => ins.empId && navigate(`/app/employee/${ins.empId}`)}
                  data-cursor={ins.empId ? 'View' : undefined}
                >
