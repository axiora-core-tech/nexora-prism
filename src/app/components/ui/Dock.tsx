...
export function Dock() {
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {expanded && (
          <motion.div
            ...
            className="flex items-center gap-1 px-3 py-2.5 p-bg-card backdrop-blur-2xl border p-border-mid rounded-full shadow-2xl shadow-black/60"
          >
            {featureNav.map((item) => (
              <NavLink
                key={item.path}
                ...
                className={({ isActive }) =>
                  `relative group flex flex-col items-center gap-1 px-2 py-1.5 rounded-full transition-all ${
                    isActive ? 'text-white' : 'text-white/40 hover:p-text-hi hover:bg-white/5'
                  }`
                }
              >
                {({ isActive }) => (
      ...
      {/* Main dock */}
      <div className="flex items-center gap-1 px-3 py-2.5 p-bg-card backdrop-blur-2xl border p-border-mid rounded-full shadow-2xl shadow-black/50">
        {primaryNav.map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
          ...
          >
            {({ isActive }) => (
              <>
                ...
                {isActive && (
                  <>
                    <motion.div layoutId="dock-bg" className="absolute inset-0 p-bg-pill rounded-full z-0" transition={{ type: "spring", stiffness: 300, damping: 30 }} />
        ...
        <div className="w-px h-5 p-bg-pill mx-1" />

        <button
          onClick={() => setExpanded(!expanded)}
          className={`relative p-2.5 rounded-full transition-all duration-300 ${
            expanded ? 'text-amber-400 bg-amber-400/10' : 'text-white/40 hover:p-text-hi hover:bg-white/5'
          }`}
